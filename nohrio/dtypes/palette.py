...
class MasterPalette(IOHandler, np.ndarray):
    ...
    def _save(self, fh, newformat=False):
        if not newformat:
            if self.ndims > 2:
                raise ValueError('Can\'t save multiple (%d) master palettes'
                                 ' in old format' % self.shape[0])
            self = self.astype('h')
            bsave(self.data, fh)
            return
        self = self[..., :3]
...
class SpritePalette(IOHandler, np.ndarray):
    ...
    def apply(self, sprite, masterpalette):
        """Convert an 8bit indexed sprite to 32bit rgba
        by applying the palette.
        ...
        with rpg.openlump('palettes.bin', 'rb') as fh:
            ...
            if hdrsize != 4:
                print('WARNING: remainder of master palette header'
                      'was not loaded (%d bytes of unknown'
                      ' data)' % (hdrsize - 4))
                fh.seek(hdrsize, 0)
...
#    master = loadmaster()
    ...
    return loadmasterpalettes(rpg), loadspritepalettes(rpg)


if __name__ == "__main__":
    with Filelike('../../ohrrpgce/vikings/vikings.rpgdir/viking.pal',
                  'rb') as fh:
        fh.seek(16 * 10)
        pal = SpritePalette(source=fh)
        assert ((pal == [0, 145,  49, 147,  52, 150, 248,
                         7, 153,  54, 252, 253, 240, 159, 218,  15]).all())
    with Filelike('../../ohrrpgce/vikings/vikings.rpgdir/palettes.bin',
                  'rb') as fh:
        fh.seek(4)
