...
class Filelike (object):
    "Wrapper context-manager which acts like open() when input is a filename,"
    " and leaves the handle unchanged otherwise"""
...
def FilelikeLump(infile, lumpname, *args, **kwargs):
    if type(infile) == str:
        if not os.path.isdir(infile):
            raise ValueError('Attempting to open lump file inside %r'
                             ', but it isn\'t a directory!' % infile)
    ...
    if not dtype.names:
        ...
        if char in 'egFDGUSVOMm':
            if (not can_simplify) or char in 'gFDGUVO':
                raise ValueError("%r cannot be converted"
                                 " to a struct code" % dtype)
